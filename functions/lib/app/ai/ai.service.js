...
let AIService = AIService_1 = class AIService {
    constructor(translationService) {
        ...
        this.systemPrompt = `You are Alex, a senior technology consultant at WatchThis - a premium digital solutions agency. You're known for being insightful, warm, and genuinely helpful. Think of yourself as a trusted advisor, not a salesperson.
...
Remember: You're not here to "close deals" - you're here to help people make smart decisions. If you do that well, the deals close themselves. Be the consultant you'd want to talk to if YOU needed help with technology. Let's do this! 🚀`;
    ...
    async generateResponse(request) {
        ...
        this.logger.log(`AI request: "${message.substring(0, 50)}..." in language: ${language} (history: ${(conversationHistory === null || conversationHistory === void 0 ? void 0 : conversationHistory.length) || 0} messages)`);
        try {
            ...
            if (conversationHistory && language !== 'en') {
                englishHistory = await Promise.all(conversationHistory.map(async (msg) => ({
                    role: msg.role,
     ...
     */
    async getAIResponse(message, context, conversationHistory) {
        var _a, _b, _c, _d, _e;
        if (!this.apiKey) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new common_1.HttpException(`Gemini API returned status ${response.status}`, common_1.HttpStatus.BAD_GATEWAY);
            }
            const data = await response.json();
            if (data.error) {
                throw new common_1.HttpException(`Gemini API error: ${data.error.message}`, common_1.HttpStatus.BAD_REQUEST);
            }
            // Ekstraktovanje odgovora iz Gemini formata
            const aiText = ((_e = (_d = (_c = (_b = (_a = data.candidates) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.content) === null || _c === void 0 ? void 0 : _c.parts) === null || _d === void 0 ? void 0 : _d[0]) === null || _e === void 0 ? void 0 : _e.text) || '';
            if (!aiText) {
            ...
            throw new common_1.HttpException('Failed to generate AI response', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    /**
    ...
    analyzeIntent(message) {
        ...
        if (hasAction && intents.booking > 0)
            intents.booking += 2;
        if (hasQuestion && intents.pricing > 0)
            intents.pricing += 1;
        if (hasQuestion && intents.services > 0)
            intents.services += 1;
        // Entity extraction
        const entities = {};
     ...
     */
    generateContextualResponse(intent, confidence, entities, originalMessage) {
        let response = '';
        switch (intent) {
            case 'booking':
    ...
    async getFallbackResponse(message, language) {
        ...
        const fallbackText = this.generateContextualResponse(analysis.intent, analysis.confidence, analysis.entities, message);
